public class GeneticAlgorithm{
    ...
    public double calcFitnes(Individual individual){
        int correctGenes=0;
        for(int i=0;i<individual.getChromosomeLenth();i++){
            if(individual.getGene(i)==1){
                correctGenes+=1;
            }
        }
        double fitness=(double)correctGenes/individual.getChromosomeLenth();
    ...
    public void evalPopulation(Population population){
        ...
        for(Individual individual:population.getIndividuals()){
            populationFitness+=calcFitnes(individual);
        }
    ...
    public Population crossoverPopulation(Population population){
        ...
        for(int i=0;i<population.size();i++){
            ...
            if(this.crossoverRate>Math.random() && i>=this.elitismCount){
                Individual offspring=new Individual(parent1.getChromosomeLenth());
                Individual parent2 = selectParent(population);
                for(int j=0;j<parent1.getChromosomeLenth();j++){
                    if(0.5>Math.random()){
    ...
    public Population mutatePopulation(Population population){
        ...
        for(int i=0;i<population.size();i++){
            ...
            for(int j=0;j<individual.getChromosomeLenth();j++){
